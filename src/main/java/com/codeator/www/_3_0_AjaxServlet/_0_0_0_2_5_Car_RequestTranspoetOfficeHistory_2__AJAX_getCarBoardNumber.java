 ...
 * @author Mahmoud
 */
public class _0_0_0_2_5_Car_RequestTranspoetOfficeHistory_2__AJAX_getCarBoardNumber extends HttpServlet {
...
//dd-MMM-yyyy
            ...
            String HTML_DIV = "";
            List<VcleRecordMovement> vList = new VcleRecordMovementDAO().GetVcleRecordMovementCarsInPeriod(FromDate, ToDate);
            if (vList.isEmpty()) {
                HTML_DIV = "NoData";
            } else {
                //int i = 0;
                Set<String> boardNumber = new HashSet<>();
                HTML_DIV = "<option value=''>اختر رقم السيارة ...</option>";
                for (VcleRecordMovement v : vList) {
                    //System.out.println(i);
                    if (boardNumber.add(v.getVcleVehicleMaster().getBoardNumber())) {
                        HTML_DIV += "<option value='" + v.getVcleVehicleMaster().getBoardNumber() + "'>" + v.getVcleVehicleMaster().getBoardNumber() + "-أقصى عدد للركاب-" + v.getVcleVehicleMaster().getVcleVehicleType().getGuestNo() + " </option>";
                    }
                    //i++;
                }
            }
