...
import com.codeator.www._1_2_DB_entities.InvoDeptUnitJoinId;
 ...
 * @author Moham
 */
public class _1_0_0_0_9_Invo_Item_Tax_Unit_Join_04_saveUnits_AjaxServlet extends HttpServlet {
    ...
    protected synchronized void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            _201_units_in_dept[] unitsSelected = mapper.readValue(json, _201_units_in_dept[].class);
            ...
            InvoDeptUnitJoinDAO dao = new InvoDeptUnitJoinDAO();
            List<InvoDeptUnitJoin> pList = dao.GetSelectedUnitInDept(accUnitId, sectionId, deptId);
            new HibernateSessionCon().beginTrans();
            for (InvoDeptUnitJoin p : pList) {
                dao.delete(p);
            }
            for (_201_units_in_dept unitSelected : unitsSelected) {
                InvoDeptUnitJoin p = new InvoDeptUnitJoin();
                InvoDeptUnitJoinId id = new InvoDeptUnitJoinId(
                        unitSelected.getUnitCode(),
                        Long.parseLong(unitSelected.getDeptId()),
                        Integer.parseInt(unitSelected.getSectionId()),
                        accUnitId);
                p.setId(id);

                List<InvoUnitType> untiList = new InvoUnitTypeDAO<>().GetInvoUnitTypeByUnitTypeCode(unitSelected.getUnitCode());
                if (untiList.isEmpty()) {
                    throw new Exception();
                }
                InvoUnitType invoUnitType = untiList.get(0);
                p.setInvoUnitType(invoUnitType);
                p.setInvoAccUintDept(deptOld);

                p.setUnitNickname(unitSelected.getUnitNickName());
                dao.insert(p);
            }
