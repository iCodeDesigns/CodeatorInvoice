 ...
 * @author Moham
 */
public class _9_9_9_9_9_7_Application_Menu_Index_Sort_Ajax_4_SaveApplicationTabsIndex_AjaxServlet extends HttpServlet {
    ...
    protected synchronized void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (tabId.length == 0 || index.length == 0) {
                throw new Exception();
            }
            new HibernateSessionCon().beginTrans();

            for (int i = 0; i < tabId.length; i++) {
                String ID[] = tabId[i].split("-");
                String applicationId_St = ID[0];
                String tabId_Str = ID[1];

                List<AppTabs> pList = new AppTabs_DAO().get_ApplicationTabsById(applicationId_St, tabId_Str);
                AppTabs h = pList.get(0);

                h.setTabIndex(Integer.parseInt(index[i]));
