...
function getInvoiceCountNumbers() {
    ...
    if (docStatus === "") {
        ...
        Warning("برجاء اختيار حالة المستند");
        return;
    } else if (searchType === "") {
        Warning("برجاء اختيار نوع البحث");
        return;
    } else if (searchType === "d" && date === "") {
        Warning("برجاء اختيار يوم");
        return;
    } else if (searchType === "m" && month === "") {
        Warning("برجاء اختيار شهر");
        return;
    ...
    var sectionOld = dept.split("-")[0];
    var deptOld = dept.split("-")[1];
    $.ajax({
        url: "_1_0_0_0_7_Electronic_Invoice_Dept_Charts_01_get_invoice_numbers?"
